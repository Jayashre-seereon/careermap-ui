...
function Particle({ x, y, size, opacity, duration, delay }: typeof PARTICLES[0]) {
...
function PencilParticle({ x, y, size, opacity, duration, delay, rotate }: typeof PENCILS[0]) {
  const anim = useRef(new Animated.Value(0)).current;
...
function BookParticle({ x, y, size, opacity, duration, delay }: typeof BOOKS[0]) {
  const anim = useRef(new Animated.Value(0)).current;
    ...
    <Animated.View style={{
      ...
      <Svg width={totalW} height={totalH}>
        ...
        {lineRows.map((frac, i) => (
          <Line
            key={`ll${i}`}
            x1={size * 0.15}  y1={pageH * frac}
            x2={pageW * 0.85} y2={pageH * frac - size * 0.05}
            stroke="#ffffff"
            strokeWidth={1.2}
            opacity={0.3}
          />
        ...
        {lineRows.map((frac, i) => (
          <Line
            key={`rl${i}`}
            x1={pageW + spineW + size * 0.15} y1={pageH * frac - size * 0.05}
            x2={pageW * 2}                    y2={pageH * frac}
            stroke="#ffffff"
            strokeWidth={1.2}
            opacity={0.3}
          />
...
export function AnimatedBackground() {
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      ...
      {/* Pencils */}
      {PENCILS.map((p, i) => (
        <PencilParticle key={i} {...p} />
      ))}
      {/* Open books */}
      {BOOKS.map((p, i) => (
        <BookParticle key={i} {...p} />
      ))}
    </View>
  );
}
