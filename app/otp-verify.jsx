...
export default function OtpVerifyScreen() {
    ...
    return (<SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }}> 
        ...
        <TextInput value={otp} onChangeText={(value) => setOtp(value.replace(/\D/g, '').slice(0, 4))} keyboardType="number-pad" placeholder="0000" placeholderTextColor={palette.muted} className={`h-[60px] w-full max-w-[220px] rounded-[18px] border px-4 text-center text-[22px] font-extrabold tracking-[12px] ${preferences.darkMode ? 'border-[#1a1a1a] bg-[#080808] text-white' : 'border-line bg-card text-ink'}`}/>
