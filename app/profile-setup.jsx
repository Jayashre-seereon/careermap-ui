...
export default function ProfileSetupScreen() {
    ...
    return (<SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }}> 
        ...
        {hasOnboardingSelections ? (<View className="gap-[14px] rounded-[22px] border border-line bg-card p-[18px]">
            ...
            {onboarding.userType ? (<View className="flex-row flex-wrap gap-2.5">
                ...
                {onboarding.childName ? (<View className="flex-row items-center gap-1.5 rounded-full bg-paper px-3 py-2">
            ...
            <View className="flex-row flex-wrap gap-2.5">
              {onboardingChips.map((item) => (<View key={`${item.label}-${item.icon}`} className="flex-row items-center gap-[7px] rounded-full bg-paper px-3 py-[9px]">
        ...
        ].map((gender) => (<AnimatedPressable key={gender.label} onPress={() => update('gender', gender.label)} className={`h-12 flex-1 px-3 py-2 flex-row items-center justify-center gap-1.5 rounded-[16px] border ${form.gender === gender.label ? 'border-brand bg-brand' : 'border-line bg-card'}`}>
        ...
        <View className="gap-1.5">
          ...
          <View className="h-14 flex-row items-center gap-2.5 rounded-[18px] border border-line bg-card px-4">
            ...
            <TextInput value={form.dob} onChangeText={(value) => update('dob', value)} placeholder="YYYY-MM-DD" placeholderTextColor={palette.muted} className="flex-1 text-[15px] text-ink"/>
          </View>
        </View>
