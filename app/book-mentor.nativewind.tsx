    ...
    return (
      <Screen>
        <SectionHeader
          ...
          action={
            <Pressable className="h-[38px] w-[38px] items-center justify-center rounded-[12px] bg-[#f2ebe6]" onPress={() => setProcessing(false)}>
    ...
    return (
      <Screen>
        <SectionHeader
          ...
          action={
            <Pressable className="h-[38px] w-[38px] items-center justify-center rounded-[12px] bg-[#f2ebe6]" onPress={() => setShowPayment(false)}>
              <Ionicons name="arrow-back" size={18} color={palette.text} />
            </Pressable>
          }
        />
        ...
        {selectedPayment === 'upi' ? (
          <TextInput value={upiId} onChangeText={setUpiId} placeholder="yourname@upi" placeholderTextColor={palette.muted} className="rounded-[16px] border border-line bg-card px-4 py-[14px] text-[14px] text-ink" />
        ) : null}

        {selectedPayment === 'netbanking' ? (
          <View className="flex-row flex-wrap gap-2.5">
            {['SBI', 'HDFC Bank', 'ICICI Bank', 'Axis Bank'].map((bank) => (
              <View key={bank} className="min-w-[47%] items-center rounded-[14px] border border-line bg-card py-3">
      ...
      <Screen>
          ...
          <View className="flex-row flex-wrap gap-2.5">
            {dates.map((date) => (
              <Pressable
                ...
                className="w-[62px] items-center gap-0.5 rounded-[16px] py-2.5"
                style={{
        ...
        {selectedDate ? (
          <View className="gap-2 rounded-[24px] border border-line bg-card p-5">
            ...
            <View className="flex-row flex-wrap gap-2.5">
              {slots.map((slot) => (
                <Pressable key={slot} className="rounded-[12px] px-[14px] py-2.5" onPress={() => setSelectedSlot(slot)} style={{ backgroundColor: selectedSlot === slot ? palette.primary : '#f2ebe6' }}>
    ...
    <Screen>
      ...
      <View className="gap-3">
        {mentors.map((mentor, index) => (
          <Pressable key={mentor.name} className="flex-row items-center gap-3 rounded-[22px] border border-line bg-card p-4" onPress={() => setSelectedIndex(index)}>
            <View className="h-[52px] w-[52px] items-center justify-center rounded-[18px]" style={{ backgroundColor: `${mentor.accent}15` }}>
              <Text className="text-[20px] font-black" style={{ color: mentor.accent }}>{mentor.avatar}</Text>
            </View>
