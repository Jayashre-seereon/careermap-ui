...
export default function InstituteScreen() {
    ...
    if (selectedIndex !== null) {
        ...
        <SectionHeader title={item.name} subtitle="Institute detail view shaped to match the reference prototype." action={<AnimatedPressable className={`h-[38px] w-[38px] items-center justify-center rounded-[12px] ${preferences.darkMode ? 'bg-[#211927]' : 'bg-[#f2ebe6]'}`} onPress={() => setSelectedIndex(null)}>
        ...
        <View className={`gap-2.5 rounded-[22px] border p-4 ${preferences.darkMode ? 'border-[#2d2430] bg-[#211927]' : 'border-line bg-card'}`}>
        ...
        <View className={`gap-2.5 rounded-[22px] border p-4 ${preferences.darkMode ? 'border-[#2d2430] bg-[#211927]' : 'border-line bg-card'}`}>
          ...
          <View className="flex-row flex-wrap gap-2">
            {item.courses.map((course) => (<View key={course} className={`rounded-[12px] px-3 py-2 ${preferences.darkMode ? 'bg-[#2a2230]' : 'bg-[#f7f2fb]'}`}>
    ...
    return (<Screen animationKey={animationKey}>
      <SectionHeader title="Institutes" subtitle="Institute directory with filters and detail cards based on the reference prototype." action={<View className="flex-row gap-2">
            <AnimatedPressable className={`h-[40px] w-[40px] items-center justify-center rounded-[12px] ${showFilters ? 'bg-brand' : preferences.darkMode ? 'bg-[#211927]' : 'bg-[#f2ebe6]'}`} onPress={() => setShowFilters((value) => !value)}>
            ...
            <AnimatedPressable className={`rounded-[12px] px-2.5 py-2 ${sortAZ ? 'bg-brand' : preferences.darkMode ? 'bg-[#211927]' : 'bg-[#f2ebe6]'}`} onPress={() => setSortAZ((value) => !value)}>
      ...
      {showFilters ? (<View className="gap-2.5">
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerClassName="gap-2.5 pr-1">
            {['All', 'Engineering', 'Medical', 'Business', 'Design', 'Law'].map((label) => (<AnimatedPressable key={label} className={`rounded-full px-3 py-2 ${typeFilter === label ? 'bg-brand' : preferences.darkMode ? 'bg-[#211927]' : 'bg-[#f2ebe6]'}`} onPress={() => setTypeFilter(label)}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerClassName="gap-2.5 pr-1">
            {['All', 'Engineering', 'Medical', 'Business', 'Design', 'Law'].map((label) => (<AnimatedPressable key={`career-${label}`} onPress={() => setCareerFilter(label)} className={`rounded-full px-3 py-2 ${careerFilter === label ? 'bg-brand' : preferences.darkMode ? 'bg-[#211927]' : 'bg-[#f2ebe6]'}`}>
      ...
      <View className="gap-3">
        {filtered.map((item, index) => (<AnimatedPressable key={item.name} className={`gap-3 rounded-[22px] border p-4 ${preferences.darkMode ? 'border-[#2d2430] bg-[#211927]' : 'border-line bg-card'}`} onPress={() => setSelectedIndex(index)}>
