...
export default function QuizScreen() {
    ...
    if (completed) {
        return (<Screen>
        <SectionHeader title="Quiz Complete" subtitle="Completion state and scoring now follow the prototype more closely." action={<Pressable onPress={() => {
                    setActiveQuiz(null);
    ...
    if (activeQuiz !== null) {
        ...
        <Pressable disabled={answers[currentQuestion] === null} onPress={() => {
            ...
            }} className="rounded-[16px] bg-brand py-[14px]" style={({ pressed }) => ({ opacity: answers[currentQuestion] === null || pressed ? 0.45 : 1 })}>
    ...
    return (<Screen>
      ...
      <View className="gap-3">
        {quizzes.map((quiz, index) => (<Pressable key={quiz.title} onPress={() => {
                setActiveQuiz(index);
                setCurrentQuestion(0);
                setAnswers(Array(sampleQuestions.length).fill(null));
