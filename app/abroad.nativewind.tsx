    ...
    return (
      <Screen>
        <SectionHeader
          ...
          action={
            <Pressable className="h-[38px] w-[38px] items-center justify-center rounded-[12px] bg-[#f2ebe6]" onPress={() => setShowForm(false)}>
              <Ionicons name="arrow-back" size={18} color={palette.text} />
            </Pressable>
          }
        />
        <View className="gap-[14px] rounded-[24px] border border-line bg-card p-[18px]">
          {[
            ...
            <View key={label} className="gap-1.5">
              <Text className="text-[12px] font-extrabold text-muted">{label}</Text>
              <TextInput value={value as string} onChangeText={setter as (text: string) => void} placeholder={placeholder as string} placeholderTextColor={palette.muted} className="rounded-[16px] border border-line bg-surface px-4 py-[14px] text-[13px] text-ink" />
            </View>
          ))}
    ...
    return (
      <Screen>
        ...
        {!unlocked && !detailLocked ? (
          <View className="self-start rounded-full px-3 py-2" style={{ backgroundColor: `${palette.orange}12` }}>
        ...
        {detailLocked ? (
          ...
          <>
            ...
            {[
              ...
              <View key={title as string} className="gap-2.5 rounded-[26px] border border-line bg-card p-[22px]">
                <Text className="text-[15px] font-extrabold text-brand">{title as string}</Text>
                {(lines as string[]).map((line) => (
                  <Text key={line} className="text-[13px] leading-5 text-muted">{line}</Text>
                ))}
              </View>
            ))}
